...
\title{ 
%	\logo{
Modern Digital Synchronization Techniques for Reliable Communication 
%	}
}
...
\begin{abstract}
\boldmath
A brief description about the design and implementation of Modern Digital Synchronization Techniques for reliable communication.
\end{abstract}
...
\begin{lstlisting}
https://github.com/gadepall/EE5837/raw/master/synctech/codes/Gardner_Time_sync_SNR_with_different_offsets.py
\end{lstlisting}
and shows the variation of the BER with respect to the SNR  with 
different timing offsets $\tau$.
...
\includegraphics[width=\columnwidth]{./figs/frequencyestiamtion_best_error_vs_snr.eps}
\end{center}
\caption{$\Delta f = 5$ MHz, Center frequency $f_c=25$ GHz}
\label{fig:freq_est}
\end{figure}
...
$\Delta\phi$  \cite{4}.  Then
\begin{equation}
\label{eq:phase_offset_model}
Y_k= X_k e^{j\Delta\phi} + V_k, \quad k = 1,\dots,N 
\end{equation}  
From \eqref{eq:phase_offset_model},
%
\begin{align}
Y_kX_k^*&= \abs{X_k}^2e^{j \Delta\phi}+ X_k^*{V}_k\\
\implies r_k&=e^{j\Delta\phi}+ \bar{V}_k
\end{align}
where
\begin{align}
r_k=Y_kX_k^*, \bar{V}_k=X_k^*{V}_k, \abs{X_k}^2=1
...
\hat{\Delta\phi}_k  = arg(r_k)
\end{equation} 

This equation gives the final estimation of phase
\begin{equation}
{\hat{\Delta\phi}^{(p)}}_{f}(l) = {\hat{\Delta\phi}^{(p)}}_{f}(l-1) + \alpha{SAW}[{\hat{\Delta\phi}^{(p)}}(l) -{\hat{\Delta\phi}^{(p)}}_{f}(l-1)] 
\end{equation}

 \begin{equation}
 SAW[\phi]=\sbrak{\phi}^{\pi}_{-\pi}
 \end{equation} Where $SAW$ is sawtooth non-linearity and $\alpha \leq 1$ . \\$l$ counts the number of pilot-based estimates.\\ ${\hat{\Delta\phi}^{(p)}}_{f}(l)$  is the final unwrapped pilot estimate.
...
\end{figure}
%
 The codes for generating the plots are available at

Fig. \ref{fig:phaseerrorp} shows the variation of the phase error in the offset estimate with respect to the pilot symbols.
$\Delta f$ when the SNR = 10 dB.  Similarly Fig. \ref{fig:phaseerrorsnr} shows the variation of the error with 
respect 
to the SNR for pilot symbols $P = 18$.
