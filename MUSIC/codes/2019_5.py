...
c2 = -1
polc = [c0,c1,c2]
[alpha,beta] = np.roots(polc)

n = 20
N = np.asarray(range(1,n))
a = (alpha**N - beta**N)/(alpha-beta)

#print(np.shape(a))
print(np.sum(a[0:(n-3)])+1,a[n-2])
...
#plt.plot(A[0], A[1], 'o')
#plt.text(A[0] * (1 - 0.3), A[1] * (1+0.2 ) , 'A')
#plt.plot(B[0], B[1], 'o')
#plt.text(B[0] * (1 + 0.05), B[1] * (1+0.1) , 'B')
#plt.plot(C[0], C[1], 'o')
...
##
##if using termux
#plt.savefig('../figs/2019_4.pdf')
#plt.savefig('../figs/2019_4.eps')
#subprocess.run(shlex.split("termux-open ../figs/2019_4.pdf"))
#else
#plt.show()
