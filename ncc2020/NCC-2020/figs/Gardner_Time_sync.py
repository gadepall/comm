...
Eb_N0_dB=2.5 # SNR range in dB scale

time_offsets=np.array([2,20,70,80,90])  # List of Timing Offsets
len_time_offsets=len(time_offsets)
BER_sim=np.zeros(len_time_offsets)   # Bit Error Rate Values

for i in range(len_time_offsets):
    #========================================================
    # Noise characteristics
    N0 = 1/(np.exp(Eb_N0_dB*np.log(10)/10.0))
    ...
    timing_offset=time_offsets[i]  # Timing offset addition 
...
plt.semilogy(time_offsets,BER_sim) 
plt.title("Timing Sync using Gardner TED")
plt.legend(['$\\frac{Eb}{N0}=%f$dB'%Eb_N0_dB],loc='best')
plt.xlabel('$\\tau$')
plt.ylabel('$P_e$')
plt.ylim([0.01,0.05])
plt.grid(True)
plt.savefig("./withfixedsnr_variable_timeoffsets.eps")
plt.show()
#=============================================================================================
